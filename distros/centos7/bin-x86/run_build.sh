...
log "Configure"
./configure --with-linux=${KERNELDIR}  --with-zfs=/usr/local/src/zfs-0.7.13 --with-spl=/usr/local/src/spl-0.7.13 --with-zfs-devel=/usr/local --disable-shared >>${BUILDLOG} 2>&1
RETVAL=$?
if [ $RETVAL -ne 0 ] ; then
